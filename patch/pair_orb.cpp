...
void PairOrb::compute(int eflag, int vflag)
{
    ...
    if (eflag_atom)
    {
        error->all(FLERR, "Pair style Orb Model does not support atomic energy");
    }

    if (vflag_atom)
    {
        error->all(FLERR, "Pair style Orb Model does not support atomic virial pressure");
    }
...
void PairOrb::performGNN()
{
    ...
    // perform Graph Neural Network Potential of Orb Model
...
void PairOrb::settings(int narg, char **arg)
{
    if (comm->nprocs > 1)
    {
        error->all(FLERR, "Pair style Orb Model does not support MPI parallelization");
    }
...
void PairOrb::coeff(int narg, char **arg)
{
    ...
    if (ntypesEff < 1)
    {
        error->all(FLERR, "There are no elements for pair_coeff of Orb Model.");
    }
    ...
    if (this->cutoff <= 0.0)
    {
        error->all(FLERR, "Cutoff is not positive for pair_coeff of Orb Model.");
    }
...
void PairOrb::init_style()
{
    ...
    {
        error->all(FLERR, "Pair style Orb Model requires 'units metal'");
    }
    ...
    {
        error->all(FLERR, "Pair style Orb Model requires periodic boundary condition");
    }
...
double PairOrb::initializePython(const char *name, int gpu)
{
    ...
    if (this->initializedPython == 0)
    {
        ...
        error->all(FLERR, "Cannot initialize python for pair_coeff of Orb Model.");
    }
...
double PairOrb::calculatePython()
{
    ...
    {
        error->all(FLERR, "Cannot calculate energy, forces and stress by python of Orb Model.");
    }
